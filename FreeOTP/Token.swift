...
public final class Token : NSObject, KeychainStorable {
	...
	public init?(otp: OTP, urlc: URLComponents, load: Bool = false) {
		...
		guard urlc.scheme == "otpauth" && urlc.host != nil else {
			return nil
		}
		...
		for item: URLQueryItem in query {
			...
			case "image":
				image = v
				/* FLFL: What's the use of the line below?? */
				if !load {image = v}
				
			case "issuerorig":
				if !load {issuerOrig = v}
		...
		if load {
			...
			if let i = image, i.hasPrefix("file:") && i.hasSuffix("/FreeOTP.app/default.png") {
				image = nil
			}
			if let i = imageOrig, i.hasPrefix("file:") && i.hasSuffix("/FreeOTP.app/default.png") {
				imageOrig = nil
			}
		} else {
			imageOrig = image
