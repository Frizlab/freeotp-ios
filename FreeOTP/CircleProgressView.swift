...
@IBDesignable class CircleProgressView : UIView {
	...
	override func draw(_ rect: CGRect) {
		let prog = clockwise ? progress : (1.0 - progress)
		let center = CGPoint(x: bounds.midX, y: bounds.midY)
		let radius = max(min(bounds.size.height / 2.0, bounds.size.width / 2.0) - 4, 1)
		...
		let path = UIBezierPath(arcCenter: center, radius: radius, startAngle: CGFloat(-Double.pi / 2),
										endAngle: CGFloat(radians - Double.pi / 2), clockwise: clockwise)
		if (hollow) {
			color.setStroke()
			path.lineWidth = 3.0
			path.stroke()
