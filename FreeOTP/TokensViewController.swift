...
class TokensViewController : UICollectionViewController, UICollectionViewDelegateFlowLayout, UIPopoverPresentationControllerDelegate {
	...
	override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
		...
		if let token = store.load(indexPath.row) {
			...
			ImageDownloader(cell.image.bounds.size).fromURI(token.image, completion: { image in
	...
	@objc func handleLongPress(_ gestureRecognizer:UIGestureRecognizer) {
		...
		case .changed:
			guard let currPath = currPath else {return}
			guard let lastPath = lastPath else {return}
			
			guard let cell = collectionView?.cellForItem(at: lastPath) else {return}
		...
		case .ended:
			guard let lastPath = lastPath else {break}
			...
			if let cell = collectionView?.cellForItem(at: lastPath) {
				UIView.animate(withDuration: 0.3, animations: {
					...
					cell.center = l.layoutAttributesForItem(at: lastPath)!.center
